...
import com.avrians.likesapp.databinding.ActivityMainBinding
...
class MainActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        binding.like.setOnClickListener {
            mBitmap.eraseColor(0) // Menghapus gambar sebelumnya
            showEars()
            showFace()
            showMouth(true)
            showEyes()
            showNose()
            showHair()
            showText() // Menampilkan ulang teks
            binding.imageView.setImageBitmap(mBitmap) // Menampilkan ulang gambar
        }
